...
{
	Audio::log("tauschePerVerschiebung(Audio& a1, Audio& a2)\n");
	Audio tmp = move(a1);
	a1 = move(a2);
	a2 = move(tmp);
...
void testGeschwindigkeitTauschePerVerschiebung()
{
	...
	auto ende = std::chrono::system_clock::now();
	cout << "testGeschwindigkeitTauschePerVerschiebung: " << std::chrono::nanoseconds(ende - anfang).count() / ANZAHL_SCHLEIFEN << " ns\n";

	delete a1;
	delete a2;
}

void testVerketteteZuweisung(void)
{
